...
public class EasyProgramList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try{
            InputStream is= getAssets().open("info.xls");
        ...
        try{
            InputStream is= getAssets().open("link.xls");
            Workbook wb=Workbook.getWorkbook(is);
            Sheet s=wb.getSheet(0);
            int row=s.getRows();
            int col=s.getColumns();
        ...
        ListAdapter listAdapter=new ArrayAdapter<String>(this,R.layout.row,R.id.row,progrmaList);
